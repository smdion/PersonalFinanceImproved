...
describe("computeGainLoss", () => {
  ...
  it("internal transfer between two tracked accounts nets to zero portfolio-level gain/loss", () => {
    // Regression guard for the exact bug this test block was reverted from:
    // an earlier version didn't subtract incoming rollovers, which meant a
    // same-household transfer (source outgoing + destination incoming, zero
    // real investment growth on either side) inflated total G/L by the
    // transferred amount instead of netting to zero.
    const source = computeGainLoss({
      endingBalance: 8_000,
    ...
    const destination = computeGainLoss({
      endingBalance: 2_000,
      beginningBalance: 0,
      totalContributions: 0,
      distributions: 0,
