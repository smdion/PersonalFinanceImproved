...
/** Retirement readiness page projecting savings growth, contribution strategies, and FIRE targets. */
...
export default function RetirementPage() {
  ...
  return (
    <div>
      ...
      {comparisonExpanded ? (
        comparisonLoading ? (
          <Card title="Withdrawal Strategy Comparison" className="mb-6">
        ...
        <Card
          ...
          subtitle="Compare all 8 withdrawal strategies side-by-side"
          collapsible
          defaultOpen={false}
          className="mb-6"
        >
                          ...
                          <>
                            ...
                            {decMonths ? (
                              <span className="text-xs text-amber-700 bg-amber-50 rounded px-2 py-1">
                                Weighted: {formatCurrency(decWeighted ?? 0)}/yr
                                <span className="text-[10px] text-faint ml-1">
                                  (
                                  {decMonths
                                    .map(
                                      (m, i) => `${m}mo ${decLabels[i] ?? ""}`,
                                    )
                                    .join(" +")}
                                  )
                                </span>
                              ...
                              >
                                {decLabels.map((label: string, idx: number) => (
                                  <option key={idx} value={idx}>
                                    {label} (
                                    {formatCurrency((decTotals[idx] ?? 0) * 12)}
                                    /yr)
                                  </option>
                                ))}
                              </select>
                ...
                {perPersonSettings && perPersonSettings.length > 1 ? (
                  perPersonSettings.map((ps) => (
                    <div key={ps.personId}>
                      ...
                      <div className="font-medium">
                        ...
                        <span className="text-[10px] text-faint">
                          {formatCurrency(
                            Number(ps.socialSecurityMonthly) * 12,
                          )}
                          /yr
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="font-medium">
                      ...
                      <span className="text-[10px] text-faint">
                        {formatCurrency(
                          Number(settings.socialSecurityMonthly) * 12,
                        )}
                        /yr
                      </span>
                    </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-2 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    <select
                      value={String(
                        Number(settings?.rothBracketTarget ?? "0.12"),
                      )}
                      onChange={(e) => {
                        if (!settings) return;
                        upsertSettings.mutate({
