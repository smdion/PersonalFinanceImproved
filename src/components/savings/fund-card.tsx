...
export function FundCard({
  ...
  return (
    <div className="bg-surface-primary rounded-lg border p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <div
              className="w-3.5 h-3.5 rounded-full shrink-0 ring-2 ring-surface-strong"
            ...
            {rawGoal.isEmergencyFund && (
              <span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-amber-100 text-amber-600 border border-amber-200">
                E-Fund
              </span>
            )}
          ...
          <span
            className={`text-xs font-semibold px-2.5 py-1 rounded-md w-fit ${status.bgColor} ${status.color} border ${status.borderColor}`}
          >
            {status.label}
          </span>
        </div>
        ...
        {canEdit !== false && !rawGoal.isEmergencyFund && (
          <div className="relative">
            <button
              ...
              className="text-faint hover:text-secondary text-sm px-1"
              title="Actions"
            >
              &#8943;
            </button>
      ...
      <div className="mb-3 bg-surface-sunken rounded-lg p-3 border border-subtle/50">
        ...
        <div className="flex items-baseline justify-between mb-2">
          <div className="flex items-baseline gap-2">
            ...
            {/* Show target for fixed goals and e-fund (ongoing goals have no fixed target) */}
              ...
              <>
                ...
                {canEdit !== false && !rawGoal.isEmergencyFund ? (
                  <InlineEdit
                    ...
                    formatDisplay={(v) =>
                      v === "0" ? "$0 — set target" : formatCurrency(Number(v))
                    }
          ...
          <div className="text-right">
            {(() => {
              const upcomingExpenses = transactions
                .filter(
                  (t) =>
                    t.amount < 0 && new Date(t.transactionDate) > new Date(),
                )
        ...
        <div className="mt-3 border-t pt-2">
          ...
          <div className="space-y-2">
            {childGoals.map((child) => {
              ...
              const childProgress =
                childTarget > 0 ? Math.min(childCurrent / childTarget, 1) : 0;
              return (
                      ...
                      availableParents && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          >
                            ...
                            {availableParents
                              .filter(
                                (p) => p.id !== rawGoal.id && p.id !== child.id,
                              )
                              .map((p) => (
