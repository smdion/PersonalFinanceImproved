...
-- KNOWN DATA-QUALITY ISSUE (R58, not yet resolved): the 2025 and 2026 rows
-- below are byte-identical per filing_status/w4_checkbox combination, which
-- is wrong — Pub 15-T brackets move every year. Verified by matching each
-- row's bracket residuals against real 1040 bracket boundaries: the MFJ rows
-- hold genuinely-correct 2026 data (duplicated onto the 2025 row too); the
-- Single and HOH rows hold genuinely-correct 2025 data (duplicated onto the
-- 2026 row too). Not fixed here — doing so needs real, externally-verified
-- 2025 MFJ and 2026 Single/HOH bracket tables, which were not available with
-- confidence at the time this was found. Two independent typos in the
-- existing (mislabeled) data WERE fixed below: Single/w4_checkbox=true's 6th
-- bracket baseWithholding (2099.5 -> 20099.5, dropped leading digit) and
-- HOH/w4_checkbox=true's 3rd bracket threshold (1975 -> 19750, dropped
-- digit) — both confirmed via forward-cascade arithmetic against the row's
-- own other values.
