...
| Phase                                 | Status          | Notes                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Phase 1: Skeleton + Data Model + Seed | **COMPLETE**    | Schema (<!-- AUTO-GEN:migrations -->3<!-- /AUTO-GEN --> squashed baseline per release with dialect-specific PG/SQLite SQL, <!-- AUTO-GEN:tables -->72<!-- /AUTO-GEN -->+ tables), seed script, <!-- AUTO-GEN:settingsComponents -->23<!-- /AUTO-GEN --> settings tabs                                                                                                                                  |
| Phase 2: Calculation Engine           | **COMPLETE**    | <!-- AUTO-GEN:calculators -->18<!-- /AUTO-GEN --> calculators + modular engine (<!-- AUTO-GEN:engineModules -->26<!-- /AUTO-GEN --> modules)                                                                                                                                                                                                                                                           |
...
| Phase 6: Polish + Deployment          | **IN PROGRESS** | UI review complete: shared primitives (Button, Badge, Skeleton, EmptyState), projection decomposition (8.3K→1.8K lines), responsive grids, Button adoption, Skeleton adoption, color standardization, design system docs. Docker/SWAG/monitoring/CI are built and operational (see § Deployment below) — remaining work is human ops cutover tasks (DNS, vault secrets, Semaphore template), not code. |

### What's Built

- **<!-- AUTO-GEN:pages -->30<!-- /AUTO-GEN --> pages:** Dashboard, Paycheck, Budget, Portfolio, Performance, Net Worth, Retirement (+ Methodology, Accumulation Methodology, Decumulation Methodology), Mortgage, Assets, Savings, Historical, Settings, Tools (ESPP calculator, Relocation calculator), Contributions, Data Browser, Expenses, Help, House, Liabilities, Versions, Brokerage, Analytics
- **<!-- AUTO-GEN:calculators -->18<!-- /AUTO-GEN --> calculators:** paycheck, tax, budget, contribution, mortgage, net-worth, savings, savings-capacity, efund, brokerage-goals, monte-carlo, random (distributions), relocation, coast-fire, social-security
- **Modular engine:** `lib/calculators/engine/` — <!-- AUTO-GEN:engineModules -->26<!-- /AUTO-GEN --> modules (projection orchestrator, year handlers, contribution routing, withdrawal routing, tax estimation, override resolution, balance utils/deduction, individual account tracking, growth application, RMD enforcement, post-withdrawal optimizer, spending strategies: Guyton-Klinger, Vanguard Dynamic, Constant Percentage, Endowment, Spending Decline, Forgo Inflation, RMD Spending)
- **<!-- AUTO-GEN:primaryRouters -->24<!-- /AUTO-GEN --> primary tRPC routers + <!-- AUTO-GEN:settingsSubRouters -->4<!-- /AUTO-GEN --> settings sub-routers** + shared helpers
- **<!-- AUTO-GEN:settingsComponents -->23<!-- /AUTO-GEN --> settings components**
...
**Result:** `lib/calculators/engine/` now contains <!-- AUTO-GEN:engineModules -->26<!-- /AUTO-GEN --> modules with explicit interfaces. Each module has a single responsibility and clear inputs/outputs. The orchestrator (`projection.ts`) calls modules in a defined pipeline order. The public API surface is minimal (`calculateProjection()` + `calculateContributionEngine()` + tax helpers).

See RULES.md § Engine Modularity for the full module list (<!-- AUTO-GEN:engineModules -->26<!-- /AUTO-GEN --> modules) and rules. The orchestrator calls modules in an explicit pipeline order.
...
| Engine modules          | `src/lib/calculators/engine/` (<!-- AUTO-GEN:engineModules -->26<!-- /AUTO-GEN --> modules)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Calculator I/O types    | `src/lib/calculators/types.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Config tables (declare) | `src/lib/config/` (account types, tax, RMD, IRMAA, ACA)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| DB schema               | `src/lib/db/schema.ts` (<!-- AUTO-GEN:tables -->72<!-- /AUTO-GEN -->+ tables)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| tRPC routers            | `src/server/routers/` (<!-- AUTO-GEN:primaryRouters -->24<!-- /AUTO-GEN --> primary + <!-- AUTO-GEN:settingsSubRouters -->4<!-- /AUTO-GEN --> settings sub-routers)                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Auth + tRPC middleware  | `src/server/auth.ts`, `src/server/trpc.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Pages (App Router)      | `src/app/(dashboard)/` (<!-- AUTO-GEN:pages -->30<!-- /AUTO-GEN --> pages)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Shared UI primitives    | `src/components/ui/` (<!-- AUTO-GEN:uiComponents -->24<!-- /AUTO-GEN --> components)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Domain components       | `src/components/{paycheck,budget,savings,networth,performance,mortgage,settings}/` — paycheck (<!-- AUTO-GEN:paycheckComponents -->17<!-- /AUTO-GEN -->), budget (<!-- AUTO-GEN:budgetComponents -->19<!-- /AUTO-GEN -->), savings (<!-- AUTO-GEN:savingsComponents -->23<!-- /AUTO-GEN -->), networth (<!-- AUTO-GEN:networthComponents -->9<!-- /AUTO-GEN -->), performance (<!-- AUTO-GEN:performanceComponents -->15<!-- /AUTO-GEN -->), mortgage (<!-- AUTO-GEN:mortgageComponents -->8<!-- /AUTO-GEN -->), settings (<!-- AUTO-GEN:settingsComponents -->23<!-- /AUTO-GEN -->) |
...
| Migrations              | `drizzle/` + `drizzle-sqlite/` (<!-- AUTO-GEN:migrations -->3<!-- /AUTO-GEN --> squashed baseline per release; PG in `drizzle/`, SQLite in `drizzle-sqlite/`)                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Tax-year resolution (R43):** every tax-figure consumer (the projection engine, the paycheck calculator, `savings.ts`) resolves its year through one function, `resolveTaxParams()` (`lib/config/tax-params.ts`), instead of each maintaining its own `WHERE tax_year = …` query. It picks a year — `tax_params` is an optional per-year vintage marker, not a value store; the existing `contribution_limits`/`tax_brackets`/`ltcg_brackets`/`irmaa_brackets`/`fpl_by_household` tables remain the only value store — and returns that year's slices in the shapes each engine module already consumed. A retirement profile can pin its resolved year via `retirement_profiles.tax_params_year` (NULL = track the latest enacted year, the default for every profile); re-opening a pinned profile reproduces its numbers instead of silently re-pricing when a newer tax year is seeded. The pin is reachable via `retirement.retirementProfiles.update`'s `taxParamsYear` field; there is no dedicated UI control for it yet (deliberately deferred, same as R43's test-fixture-migration scope-down — real value, separate follow-up). Also as of R43: `irmaa_brackets` is now actually read by the engine (previously the table and its Settings editor existed but nothing consumed them), and `fpl_by_household` gives the ACA subsidy cliff a DB home for the first time.
...
2. **Contribution profile** — persistent named active fields for contribution accounts (value/method/match) and deductions (amount). Salary, bonus terms, pay schedule (period/week/anchor date), and W-4 elections (filing status/checkbox/additional withholding) are NOT part of a Contribution Profile — a Contribution Profile no longer touches `jobs` at all. All of those live on the independent Salary Profile axis instead, as one complete 16-field entry per job (see `resolveCompensation`/`mergeSalaryProfileJobFields` in `server/helpers/salary.ts` and RULES.md's "The Salary Profile layer"). This collapses "how much do I earn" and "what withholding/schedule elections apply" into one axis — an accepted tradeoff, not an oversight. `employerName`'s Contribution-Profile override capability was removed the same time (no replacement) — a job's employer name is fixed once created
...
- Budget profiles select which budget column to use, not override individual values
- Savings goal allocations (%/$) vary by budget profile — `savings_goal_profile_allocations`
  is keyed to `budget_profile_id`, not independently selectable; it's a facet of
  Budget Profile, not a fourth profile type
- A Scenario/Plan can additionally **pin** which Budget Profile and/or Contribution
  Profile counts as "active" while it's selected, instead of always reading the
  single globally-active one — a reference pin (dedicated FK columns on
  `scenarios`), not a value diff, so it lives outside the `overrides` JSONB.
  Resolution precedence for "which profile is active" everywhere in the app:
  Plan pin → local page selection → globally-active profile. See
  `useEffectiveProfileId` and RULES.md's "Profile Pins" section.
...
3. **Contribution profiles and scenarios remain separate systems**: Both are named override collections, but they serve different scopes (contributions vs. whole-plan what-ifs) and compose as layers. Documented precedence: DB base → contribution profile → budget profile → scenario. A Scenario/Plan can also pin a specific Budget/Contribution Profile as "active" (see Profile Pins in RULES.md) — that pin is a reference, resolved before this override-composition order runs, not a fourth layer in it.
...
Run `ls src/components/ui/` for the live list (<!-- AUTO-GEN:uiComponents -->24<!-- /AUTO-GEN --> components). Each
