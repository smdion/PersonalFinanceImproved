-- v0.6.0 squashed schema (SQLite)
-- Generated by applying v0.5.x migrations against an in-memory SQLite DB.
-- Existing installs: db-migrate.ts detects mismatch and runs handleSQLiteSquashUpgrade().
...
CREATE TABLE `jobs` (
	...
	`budget_periods_per_month` text, `extra_paycheck_routing` text,
	FOREIGN KEY (`person_id`) REFERENCES `people`(`id`) ON UPDATE no action ON DELETE restrict
);
--> statement-breakpoint
...
CREATE TABLE `savings_allocation_overrides` (
	...
	`amount` text NOT NULL, `source` text NOT NULL DEFAULT 'manual',
...
--> statement-breakpoint
CREATE UNIQUE INDEX `api_connections_service_unique` ON `api_connections` (`service`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `asset_class_params_name_unique` ON `asset_class_params` (`name`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `budget_profiles_name_unique` ON `budget_profiles` (`name`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `contribution_profiles_name_unique` ON `contribution_profiles` (`name`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `local_admins_email_unique` ON `local_admins` (`email`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `mc_presets_key_unique` ON `mc_presets` (`key`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `retirement_settings_person_id_unique` ON `retirement_settings` (`person_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `return_rate_table_age_unique` ON `return_rate_table` (`age`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `savings_goals_name_unique` ON `savings_goals` (`name`);
--> statement-breakpoint
